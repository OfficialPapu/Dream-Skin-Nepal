...
include_once $base_url . 'Assets/PHP/Configuration/Show Selectable Product.php';
...
if (isset($_POST['CheckProductStatus'])) {
    $UserID = $_POST['UserID'];
    $RowOrderID = $_POST['RowOrderID'];
    $SatusQuery = "SELECT * FROM `orders` JOIN order_items oi ON oi.`Order ID`= orders.`Order ID` WHERE `User ID`='$UserID' AND orders.`Order ID`='$RowOrderID'";
...
if (isset($_POST['FilterByDate'])) {
    ...
    FROM `orders`
    JOIN user_table user ON user.`ID`= orders.`User ID`
    JOIN order_items OrderList ON OrderList.`Order ID`=orders.`Order ID` $Condition";
...
if (isset($_POST['OrderManagementLiveSearch'])) {
    ...
    OrderItem.`Payment Method` AS PaymentMethod,
...
WHERE pm3.`Product Meta Value`='$ProductTypeID'";
    ...
    $result = $conn->query($query);
    ShowSelectableProducts($result, $base_url, $is_mobile, $conn);
}
...
if (isset($_POST['TestResult'])) {
    ...
    $StoreInfo="INSERT INTO `skintype_test_result`(`User ID`, `Skintype`, `Date`) VALUES ('$user_id','$SkinType',CONVERT_TZ(NOW(), '+00:00', '+05:45'))";
    $StoreInfoRun=mysqli_query($conn,$StoreInfo);
    $SelectQuery = "SELECT * FROM `product_category` WHERE `Product Category Attribute` LIKE '%$SkinType%'";
    $Select = mysqli_query($conn, $SelectQuery);
   if($Select->num_rows>0){
    $Row = $Select->fetch_assoc();
    $ProductCategoryID = $Row['Product Category ID'];
    $Where = "WHERE pm3.`Product Meta Value`='$ProductCategoryID'";
   }else{
    $ProductCategoryID = 1;
    $Where = "ORDER BY RAND() LIMIT 0,25";
   }
    $query = "SELECT DISTINCT p.ID, p.`Product Title`,p.`Slug Url`, p.`Product Price`,p.`Discount Price`,p.`Discount Percentage`, pm1.`Product Meta Value` 
    AS ProductBrand, pm2.`Product Meta Value` AS ProductThumbnail, pm3.`Product Meta Value` AS ProductType,
    CASE WHEN wishlist.`Product ID` IS NOT NULL THEN 'Added' ELSE 'Not Added' END AS IsAddedToWishlist,
    ...
    JOIN postsmeta pm1 ON p.ID = pm1.`Product ID` AND pm1.`Product Meta Key` = 'Brand ID'
    JOIN postsmeta pm2 ON p.ID = pm2.`Product ID` AND pm2.`Product Meta Key` = 'Image 1'
    JOIN postsmeta pm3 ON p.ID = pm3.`Product ID` AND pm3.`Product Meta Key` = 'Skin Type'
    $Where";
    include_once $base_url . 'Assets/PHP/Configuration/Mobile Check.php';
    $result = $conn->query($query);
