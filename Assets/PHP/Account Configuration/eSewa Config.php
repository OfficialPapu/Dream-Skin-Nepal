...
$UploadDir = $base_url . "Account/UserAccount/Payment Receipts/" . date("Y/m");
if (!file_exists($UploadDir)) {
    mkdir($UploadDir, 0777, true);
}
$DBStoreName = date("Y/m") ."/". $RandNum. $_FILES["fileToUpload"]["name"];
$UploadFile = $UploadDir ."/". $RandNum. basename($_FILES["fileToUpload"]["name"]);
    ...
    if ($runquery->num_rows > 0) {
        ...
        if ($send) {
            ...
            while ($row = $runquery->fetch_assoc()) {
                ...
                $insert_into_order_items = "INSERT INTO `order_items`(`Order ID`, `User ID`, `Product ID`, `Quantity`,`Total Due`,`Payment Method`,`Payment Screenshot`, `Order Status`, `Tracking Number`, `Order Date`) VALUES ('$order_id','$user_id','$product_id_db','$product_quantity','$PerProductPrice','$PaymentMethod','$DBStoreName','Review','$trackingNumber',NOW())";
    ...
    $UpdateDnsQueryRun=$conn->query($UpdateDnsQuery);             
        ...
        WHERE
            OrderList.`Order ID` = '$order_id'";
                ...
                NotifyAdmin($UserName, $MobileNumber, $ProductTitle, $Price, $TrackingNum, $PaymentMethod, $DBStoreName);
