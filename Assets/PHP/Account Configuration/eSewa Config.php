...
$DBStoreName = date("Y/m") ."/". $_FILES["fileToUpload"]["name"];
...
if (isset($_GET['PaymentInfo'])) {
    ...
    if ($runquery->num_rows > 0) {
        $InsertOrder = "INSERT INTO `orders`(`User ID`, `Total Due`, `Shipping Fee`, `Payment Method`, `Payment Screenshot`,`Pickup`,`Notes`,`Order Date`) VALUES ('$user_id','$TotalPrice','$TotalShippingFee','$PaymentMethod','$DBStoreName','$Pickup','',$Date)";
        ...
        if ($send) {
            ...
            while ($row = $runquery->fetch_assoc()) {
                ...
                $insert_into_order_items = "INSERT INTO `order_items`(`Order ID`, `Product ID`, `Quantity`,`Total Due`,`Order Status`, `Tracking Number`, `Order Date`) VALUES ('$order_id','$product_id_db','$product_quantity','$PerProductPrice','Review','$trackingNumber',$Date)";
