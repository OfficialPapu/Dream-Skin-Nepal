...
while ($row = $RunQuery->fetch_assoc()) {
    ...
    $insertQuery = "UPDATE `product_cart` SET `Total Due`='$totalPrice', `Applied Promo Code`='' WHERE `Product_ID`='$productID'";
...
$TotalPrice = 0;
$TotalDue = "SELECT `Total Due` FROM `product_cart` WHERE `User ID`='$user_id'";
