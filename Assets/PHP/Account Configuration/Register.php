...
if (isset($_POST['DataSend'])) {
    ...
    if (empty($first_name) || empty($last_name) || empty($email) || empty($mobile_number) || empty($password)) {
        ...
        if ($already_exist_query->num_rows > 0) {
            ...
            $send_form_data = "INSERT INTO `user_table`(`ID`, `First Name`, `Last Name`, `Email`, `Mobile Number`, `Password`, `DNS Point`, `User Address`, `User Picture`, `User IP`, `User Gender`, `Date Of Birth`,`Account Signup Date`) VALUES ('','$first_name','$last_name','$email','$mobile_number','$password','','','','$ip','','',NOW())";
